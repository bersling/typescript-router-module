...
class SimpleCrudRouter {
    ...
    create(req, res, next) {
        ...
        tsmongo_1.dao.create(resource, resourceName, (dbResp) => {
            if (dbResp.error) {
                res.status(500).send({
    ...
    getOne(req, res, next) {
        ...
        const resourceName = req.params.resource;
        tsmongo_1.dao.read(resourceId, resourceName, (dbResp) => {
            if (dbResp.error) {
                res.status(500).send({
                    message: 'Server error',
                    status: res.status
    ...
    updateOne(req, res, next) {
        const resourceName = req.params.resource;
        tsmongo_1.dao.update(req.body, resourceName, (dbResp) => {
            if (dbResp.error) {
                res.status(500).send({
    ...
    getAll(req, res, next) {
        const resourceName = req.params.resource;
        tsmongo_1.dao.readAll(resourceName, (dbResp) => {
            if (dbResp.error) {
                res.status(500).send({
                    message: 'Server error',
                    status: res.status
    ...
    deleteOne(req, res, next) {
        const resourceId = req.params.id;
        const resourceName = req.params.resource;
        tsmongo_1.dao.remove(resourceId, resourceName, (dbResp) => {
            if (dbResp.error) {
                res.status(500).send({
                    message: 'Server error',
                    status: res.status
